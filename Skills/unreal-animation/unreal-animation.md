...
You have access to **Monolith** with 23 animation actions via `animation.query()`.
...
```
animation.query({ action: "get_state_machines", params: { asset_path: "/Game/Animations/ABP_Player" } })
animation.query({ action: "get_transitions", params: { asset_path: "/Game/Animations/ABP_Player", machine_name: "Locomotion" } })
```

### Get nodes with optional graph filter
```
animation.query({ action: "get_nodes", params: { asset_path: "/Game/Animations/ABP_Player" } })
animation.query({ action: "get_nodes", params: { asset_path: "/Game/Animations/ABP_Player", graph_name: "EventGraph" } })
```

### Set up montage section flow (intro -> loop -> outro)
```
animation.query({ action: "add_montage_section", params: { asset_path: "/Game/Animations/AM_Attack", name: "Intro", time: 0.0 } })
animation.query({ action: "add_montage_section", params: { asset_path: "/Game/Animations/AM_Attack", name: "Loop", time: 0.5 } })
animation.query({ action: "add_montage_section", params: { asset_path: "/Game/Animations/AM_Attack", name: "Outro", time: 1.2 } })
animation.query({ action: "set_section_next", params: { asset_path: "/Game/Animations/AM_Attack", section: "Intro", next: "Loop" } })
animation.query({ action: "set_section_next", params: { asset_path: "/Game/Animations/AM_Attack", section: "Loop", next: "Outro" } })
```

### Inspect skeleton structure
```
animation.query({ action: "get_skeleton_info", params: { asset_path: "/Game/Characters/SK_Mannequin" } })
animation.query({ action: "get_skeletal_mesh_info", params: { asset_path: "/Game/Characters/SKM_Mannequin" } })
```
